...
namespace aiko::renderer::vulkan
{
    ...
    struct SamplerStateHash
    {
        size_t operator()(const SamplerState& state) const
        {
            ...
            utils::hashCombine(
                std::hash<uint32_t>{}(static_cast<uint32_t>(state.minFilter)),
                seed
            );

            utils::hashCombine(
                std::hash<uint32_t>{}(static_cast<uint32_t>(state.magFilter)),
                seed
            );

            utils::hashCombine(
                std::hash<uint32_t>{}(static_cast<uint32_t>(state.mipFilter)),
                seed
            );

            utils::hashCombine(
                std::hash<uint32_t>{}(static_cast<uint32_t>(state.wrapU)),
                seed
            );

            utils::hashCombine(
                std::hash<uint32_t>{}(static_cast<uint32_t>(state.wrapV)),
                seed
            );
    ...
    struct MaterialBindingKeyHash
    {
        size_t operator()(const MaterialBindingKey& key) const
        {
            ...
            {
                utils::hashCombine(
                    std::hash<uint32_t>{}(texture.binding),
                    seed
                );

                utils::hashCombine(
                    std::hash<AssetId>{}(texture.textureId),
                    seed
                );

                utils::hashCombine(
                    std::hash<const Texture*>{}(texture.runtimeTexture),
                    seed
                );

                utils::hashCombine(
                    SamplerStateHash{}(texture.sampler),
                    seed
                );
            }
