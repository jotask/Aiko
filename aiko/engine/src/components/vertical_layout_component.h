...
namespace aiko
{
    ...
    class VerticalLayoutComponent : public Component
    {
    public:

        VerticalLayoutComponent();
        virtual ~VerticalLayoutComponent() = default;

        void setSpacing(float spacing)
        {
            AIKO_ASSERT(spacing >= 0.0f, "Vertical layout spacing must not be negative");
            m_spacing = spacing;
        }
        ...
        void setPadding(const UIPadding& padding)
        {
            AIKO_ASSERT(
                ...
                padding.top >= 0.0f &&
                padding.bottom >= 0.0f,
                "Vertical layout padding must not be negative");
